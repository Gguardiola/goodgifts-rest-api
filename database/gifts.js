...
const retrieveAllUserGifts = async (requestedUser, userId, limit, offset) => {
    ...
    result = await db.query('SELECT * FROM gifts WHERE id = (SELECT gift_id FROM user_gifts WHERE user_id = $2) AND gifted_user_id != $1 LIMIT $3 OFFSET $4', [requestedUser, userId, limit, offset]);
    return result;
}

const retrieveUserGift = async (userId, giftName) => {
    userId = userId.replace(/^"|"$/g, '');
    result = await db.query('SELECT * FROM gifts WHERE gift_name = $1 AND user_id = $2', [giftName, userId]);
    return result;
}

const retrieveGiftById = async (userId, giftId) => {
    userId = userId.replace(/^"|"$/g, '');
    result = await db.query('SELECT * FROM gifts WHERE id = $2 AND gifted_user_id != $1', [userId, giftId]);
    return result;
}
...
const createGift = async (userId, itemId, gifted_user_id, expiration_date, gift_name) => {
    ...
    try {
        ...
        await db.query('INSERT INTO gifts (item_id, gift_name, gifted_user_id, is_delivered, expiration_date, user_id) VALUES ($1, $2, false ,$3, $4)', [itemId, gift_name, gifted_user_id, expiration_date]);
...
const retrieveImplicationRequests = async (giftId, limit, offset) => {
    result = await db.query('SELECT * FROM user_gifts WHERE gift_id = $1 AND is_implicated = false LIMIT $2 OFFSET $3', [giftId, limit, offset]);
    return result;
}

const retrieveImplications = async (giftId, limit, offset) => {
    result = await db.query('SELECT * FROM user_gifts WHERE gift_id = $1 AND is_implicated = true LIMIT $2 OFFSET $3', [giftId, limit, offset]);
    return result;
}

const retrieveUserImplications = async (userId, limit, offset) => {
    userId = userId.replace(/^"|"$/g, '');
    result = await db.query('SELECT * FROM user_gifts WHERE user_id = $1 AND is_implicated = true LIMIT $2 OFFSET $3', [userId, limit, offset]);
    return result;
}

const retrieveUserImplicationsRequested = async (userId, limit, offset) => {
    userId = userId.replace(/^"|"$/g, '');
    result = await db.query('SELECT * FROM user_gifts WHERE user_id = $1 AND is_implicated = false LIMIT $2 OFFSET $3', [userId, limit, offset]);
    return result;
}

const retrieveImplication = async (userId, giftId) => {
    userId = userId.replace(/^"|"$/g, '');
    result = await db.query('SELECT * FROM user_gifts WHERE gift_id = $1 AND user_id = $2', [giftId, userId]);
    return result;
}
